...
> **Phạm vi:** Toàn bộ 107 permission — truy cập mọi API endpoint.
...
3. KH chọn khuyến mãi → `GET /api/v1/khuyen-mai-theo-hoa-don` + `GET /api/v1/khuyen-mai-theo-diem`
4. KH đặt hàng → `POST /api/v1/don-hang/online`
5. Hệ thống tự động: tạo đơn + chi tiết đơn → giảm tồn kho → xóa giỏ hàng
6. KH theo dõi đơn → `GET /api/v1/don-hang/{id}`
7. Khi đơn hoàn thành (trangThai=5) → hệ thống tự cộng điểm tích lũy
